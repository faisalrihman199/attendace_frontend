...
const AddAthlete = () => {
  ...
  const { allTeams, generatePin, addStudent, checkPin, oneAthlete } = useAPI();
  const [pinLength, setLength] = useState(0);
  let server = import.meta.env.VITE_APP_API_URL || '';
  const navigate = useNavigate();
  const location = useLocation();

  // Message Shown toggles (array-backed)
  const [showMsgCheckin, setShowMsgCheckin] = useState(true);
  const [showMsgEmail, setShowMsgEmail] = useState(true);
  ...
  useEffect(() => {
    if (athlete) {
      ...
      // Init array-based "messageShown" -> toggles
      console.log("Message shown :", athlete?.messageShown);
      
      const arr =  athlete.messageShown ;
      setShowMsgCheckin(arr ? arr.includes('check-in') : true);
      setShowMsgEmail(arr ? arr.includes('email') : true);
  ...
  const onSubmit = (data) => {
    ...
    const messageShown = [];
    if (showMsgCheckin) messageShown.push('check-in');
    if (showMsgEmail) messageShown.push('email');
    ...
    formData.append('messageShown', JSON.stringify(messageShown));

    if (photo) {
      formData.append('photo', photo);
    }

    console.log("Message Shown form:",Object.fromEntries(formData))
    addStudent(formData)
      .then((res) => {
        if (res.success) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className='poppins-regular'>
      ...
      {/* Message Shown (array-backed) */}
      <div className="row mb-3">
        <div className="col-sm-12">
          <label className="form-label mb-1">Message Shown</label>

          <div className="d-flex align-items-center justify-content-between bg_dede rounded p-2 mb-2">
            <span className="ms-1">Check-in</span>
            <IconButton
              onClick={() => setShowMsgCheckin(prev => !prev)}
              style={{ color: showMsgCheckin ? '#247BA0' : 'gray' }}
            >
              {showMsgCheckin ? <ToggleOn fontSize="large" /> : <ToggleOff fontSize="large" />}
            </IconButton>
          </div>

          <div className="d-flex align-items-center justify-content-between bg_dede rounded p-2">
            <span className="ms-1">Email</span>
            <IconButton
              onClick={() => setShowMsgEmail(prev => !prev)}
              style={{ color: showMsgEmail ? '#247BA0' : 'gray' }}
            >
              {showMsgEmail ? <ToggleOn fontSize="large" /> : <ToggleOff fontSize="large" />}
            </IconButton>
          </div>
        </div>
      </div>
